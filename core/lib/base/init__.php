...
// 	var_dump($urlInfo);
	...
	* 本系统用于输出到客户端的页面默认存放于/core/tpl/other/ 目录，要自定义存放位置，则需配置 [ 模板目录路径 OTHER_TEMPLATE_DIR  @  /core/lib/base/env__.php ]。
	* 以上输入的链接将默认访问到 /core/tpl/other/abc.php 。
	...
	*/
	if(null != $urlInfo['params'] && 0 != count($urlInfo['params']) && null != $urlInfo['params'][0]){
		...
		if(false === UrlUtil::isUrlShell($param)){
			 ...
			 * 只要请求的文件名参数能够被系统识别出a-b-c-d的格式，就可以默认访问到core/tpl/a/b/c/d.php。
